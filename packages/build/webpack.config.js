...
module.exports = function(fn) {
  return () => {
    ...
    const isSentryRequired = !!process.env.SENTRY_PROJECT;
    ...
    configs = {
      ...
      // 是否需要分离vendor块
      shouldSplitVendorChunk:
        typeof configs.shouldSplitVendorChunk === 'boolean'
          ? configs.shouldSplitVendorChunk
          : true,
      ...
      // 是否禁用入口文件的md5 hash
      disableEntryFileMD5Hash: !!configs.disableEntryFileMD5Hash,

      // 是否禁止输出`manifest` chunk
      disableManifestChunk: !!configs.disableManifestChunk,
    };
    ...
    return {
      ...
      output: {
        ...
        // 如果禁用了入口文件的md5 hash，则直接输出
        filename: `[name]${
          isDev || configs.disableEntryFileMD5Hash ? '' : '.[contenthash:10]'
        }.js`,
      },
      ...
      devtool: isDev || isSentryRequired ? 'source-map' : false,
